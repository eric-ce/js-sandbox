...
class MultiDistance {
    ...
    initializeMeasurement() {
        ...
        this.button.className = "multi-distance cesium-button"
        this.button.innerHTML = "Multi-D";
        document.body
        ...
        if (Cesium.defined(pickedObject)) {
            ...
            if (this.pointEntities.values.length > 1) {
                ...
                const labelEntity = this.viewer.entities.add(
                    createDistanceLabel(prevPointCartesian, currPointCartesian, distance)
                );
                labelEntity.label.text = formatDistance(distance);
    ...
    handleMultiDistanceMouseMove(movement) {
        ...
        if (Cesium.defined(pickedObject)) {
            ...
            if (this.pointEntities.values.length > 0) {
                ...
                this.movingLabelEntity = this.viewer.entities.add(createDistanceLabel(
                    cartesian,
                    cartesian,
                    totalDistance,
                ));
                this.movingLabelEntity.label.showBackground = false;

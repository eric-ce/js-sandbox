    ...
    _getContextMenuItemsForAnnotation(pickedObject, pickedObjectType) {
        ...
        switch (pickedObjectType) {
            ...
            case "point":
                ...
                // -- Resume measure action -- 
                // If the point is the first or last point in the measure, allow resuming the measure
                const resumeContext = this._getPointContextForResume(point);
                if (resumeContext) {
                    const { pointIndex, measureData } = resumeContext;
                    itemList.push({
                        text: "Resume measure",
                        event: () => { this._resumeMeasure(pointIndex, measureData) }
                    });
                }
                break;
            case "line":

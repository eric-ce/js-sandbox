...
class Profile extends MeasureModeBase {
    ...
    startMeasure() {
        ...
        if (this.coords.cache.length === 2) {
            ...
            // update the interpolated points for current measure
            this.measure.interpolatedPoints = clampedPositions;

            // create and or update the chart
            this.handleChartSpecific(this.measure.interpolatedPoints, this.measure.coordinates);
    ...
    handleHoverHighlighting(pickedObject) {
        super.handleHoverHighlighting(pickedObject, "profile");
    }

...
import {
    ...
    FireTrail,
    FlyThrough
} from "../measure-modes/index.mjs";
import { removeInputActions, makeDraggable, createGroundPolylinePrimitive, convertToCartesian3, createPointPrimitive, createPolylinePrimitive, createPolylinePrimitivesFromArray, createPointPrimitivesFromArray, createLabelPrimitive, createLabelPrimitivesFromArray, createPolygonPrimitive, convertToCartographicRadians, convertToCartographicDegrees } from "../lib/helper/cesiumHelper.mjs";
import { toolIcon, pickerIcon, pointsIcon, distanceIcon, curveIcon, heightIcon, multiDImage, multiDClampedIcon, polygonIcon, profileIcon, profileDistancesIcon, clearIcon, helpBoxIcon, logBoxIcon } from '../assets/icons.mjs';
...
export default class CesiumMeasure extends MeasureComponentBase {
    ...
    async initializeMeasureTools() {
        ...
        const modes = [
            ...
            // },
            {
                instance: new TwoPointsDistanceCesium(
                    this.viewer,
                    this.handler,
                    this.stateManager,
                    this.cesiumPkg,
    ...
    deactivateButton(button, toolInstance) {
        ...
        this.viewer.scene.primitives._primitives.filter(p =>
            ...
            (p.id.includes("moving") || p.id.includes("pending") || p.id.includes("line_initiate") || p?.status === "pending" || p?.status === "moving")
        ...
        this.labelCollection._labels.filter(l =>
            l &&
            l.id &&
            (l.id.includes("moving") || l.id.includes("pending") || l?.status === "pending" || l?.status === "moving")
        ...
        this.pointCollection._pointPrimitives.filter(p =>
            p &&
            p.id &&
            (p.id.includes("moving") || p.id.includes("pending") || p?.status === "pending" || p?.status === "moving")
    ...
    _addPointMarker(position, options = {}) {
        ...
        const hasHeight = position.height > 0;
        const pointPosition = hasHeight ? position : this._getClampedPositions([position])[0];
    ...
    _addPolyline(positions, options = {}) {
        ...
        const polylinePrimitive = this.viewer.scene.primitives.add(polyline);
    ...
    _addPolylinesFromArray(positions, options = {}) {
        ...
        const polylinePrimitives = polylinesArray.map(polyline => this.viewer.scene.primitives.add(polyline));
    ...
    _addPolygon(positions, options = {}) {
        ...
        const polygonPrimitive = this.viewer.scene.primitives.add(polygon);
    ...
    _getClampedPositions(positions) {
        const clampedPositions = positions.map(pos => {
            ...
            // Get ground height
            const height = this.viewer.scene.sampleHeight(cartographic) || 0;
